class Point:
    ...
    def approx_equals(self, other):
        """Whether or not this Point is approximately equal to the given one.
        Args:
            other (Point): The Point to compare
        
        Returns:
            boolean: True if both x and y are approximately equal; false if otherwise.
        """
        # if within 10 above, and 10 below, and x is equal
        return self._x == other.get_x() and ((self._y < other.get_y()+10 and self._y >= other.get_y()) or self._y > other.get_y()-10 and self._y <= other.get_y())

    def y_greater_than(self, other):
        """Whether or not this Point's y position is greater than the value of 'other'
        Args:
            other (int): the height to compare
        """
